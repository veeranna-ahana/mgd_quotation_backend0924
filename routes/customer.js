        ...
        for (let i = 0; i < olddata.length; i++) {
           ...
           //         console.log(asmdata);
                    ...
                    console.log("asmprtdata "+asmprtdata[0].Id);
                        if (err) logger.error(err);
                      //  console.log("asmprtdata " + asmprtdata[i].Id)
                        misQueryMod(`Select * from magodmis.cust_assy_bom_list  where Cust_AssyId ='${asmdata[0].assyid}' and Cust_Bom_ListId='${asmprtdata[0].Id}'`, (err, bomdata) => {
                            if (err) logger.error(err);
                            if (bomdata.length>0) {
                                res.send({ status: 'Duplicate' });
                            }
