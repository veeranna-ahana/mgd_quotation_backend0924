...
mtrlgradesRouter.post(`/getmtrlgradbymaterial`, async (req, res, next) => {
    console.log("getmtrlgradbymaterial" + req.body.mtrltype);
    try {
        const mtrl = req.body.mtrltype;
        console.log( req.body.mtrltype);
        misQuery(`Select * from magodmis.mtrlgrades WHERE Material = '${req.body.mtrltype}'`, (data) => {
            res.send(data)
        })
    } catch (error) {
        next(error)
