...
const { misQueryMod, setupQuery, setupQueryMod,qtnQueryMod, misQuery, mchQueryMod } = require('../helpers/dbconn');
...
ordersRouter.post(`/savecreateorder`, async (req, res, next) => {
    ...
    try {
        ...
        await setupQueryMod(`SELECT * FROM magod_setup.magod_runningno WHERE SrlType='Order' AND UnitName='Jigani' ORDER BY Id DESC LIMIT 1`,
            async (err, runningNoResult) => {
                ...
                let ordno = `${zzz.getFullYear().toString().substr(-2)}${(parseInt(runningno) + 1).toString().padStart(3, '0')}`;
                console.log(ordno);
                ...
                await misQueryMod(`INSERT INTO magodmis.order_list(order_no, order_date, cust_code, contact_name, Type, delivery_date, purchase_order,
            ...
            '${Transportcharges}', '${ordertype}', '${0}', '${qtnno}')`, (err, insertResult) => {
                    if (err) logger.error(err); 
                    console.log("Inserted order data:", insertResult);

                    //   if (insertResult.affectedRows === 1) {
                    // Update the running number for orders
                    setupQuery(`UPDATE magod_setup.magod_runningno SET Running_No = Running_No + 1 WHERE SrlType='Order' AND Id = ${runningNoResult[0]["Id"]}`, (err, updateResult) => {});
...
ordersRouter.post(`/getqtnnossentdata`, async (req,res,next) => {
    try {
        ...
        qtnQueryMod(`SELECT QtnNo FROM magodqtn.qtnlist WHERE QtnStatus = 'Qtn Sent' And QtnFormat = '${ordtype}' order by QtnID desc`, async (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        });
    }catch(error){
...
ordersRouter.post(`/getcmbschedulelistdata`, async (req, res, next) => {
    try {
        console.log("Getting combined schedule data");
        //const custcode = req.body.custcode;
        //      console.log(custcode);
        misQueryMod(`SELECT o.*, c.Cust_name,c1.cmbSchID 
        FROM  magodmis.orderschedule o,magodmis.cust_data c,magodmis.combined_schedule c1 
        WHERE o.ScheduleType='Combined' AND (o.Schedule_Status='Tasked' OR o.Schedule_Status='Programmed' 
        OR o.Schedule_Status='Production' OR o.Schedule_Status='Processing' OR o.Schedule_Status='Completed' Or o.Schedule_Status='Inspected')
        AND o.Cust_Code=c.Cust_Code AND o.ScheduleId=c1.ScheduleID;`, (err, cmbSchListData) => {
            console.log(cmbSchListData)
            res.send(cmbSchListData)
        })
    } catch (error) {
        next(error)
    }
...
console.log(sql);      
        ...
        //} else if (ordstatus = "Processing") {
            sql = sql + ` AND Not ( order_status ='created' OR order_status ='Suspended' OR order_status ='Cancelled' OR order_status ='Closed' `;
            sql = sql + ` OR order_status ='Closed' OR order_status ='ShortClosed' OR order_status ='Completed' OR order_status ='Suspended'  OR order_status ='Dispatched'  ) `;
...
ordersRouter.post(`/getordercustomer`, async (req, res, next) => {
    try {
        console.log("Getting order customer");
      console.log(req.body.schNo);
        await misQueryMod(`SELECT * FROM magodmis.order_list WHERE Order_No = '${req.body.schNo}'`, async (err, ordercustdata) => {
            console.log(ordercustdata)
            res.send(ordercustdata)
        })
    } catch (error) {
        next(error)
    }
...
ordersRouter.post(`/getorderstatuslist`, async (req, res, next) => {
    try {
        ...
        const orderno = req.body.ordno;
        misQueryMod("SELECT m.* FROM magod_setup.magod_statuslist m WHERE m.`Function` = 'Order' order by Id asc", (err, orderstatuslist) => {
