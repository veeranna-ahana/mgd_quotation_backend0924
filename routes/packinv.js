...
packinvRouter.post(`/getpackingschedules`, async (req, res, next) => {
    try {
        ...
        misQueryMod(`SELECT o.* FROM magodmis.orderschedule o
        ...
        AND o.Type = '${intype}'  AND o.Cust_code= '${ccode}' ORDER BY o.ScheduleDate desc`, async (err, data) => {
            if (err) {
                console.log(err);
...
packinvRouter.post(`/getpackschdetails`, async (req, res, next) => {
    try {
        ...
        misQueryMod(`SELECT o.*,c.cust_name FROM magodmis.orderschedule o, magodmis.cust_data c 
           WHERE o.ScheduleId = '${schid}' AND c.Cust_code=o.Cust_Code`, async (err, data) => {
            if (err) {
                console.log(err);
                res.status(500).send(err);
            }
...
packinvRouter.post(`/getpckscheduledetails`, async (req,res, next) => {
    try {
        ...
        setupQuery(`SELECT o.*,if (a.InDraftPn is null,0,a.InDraftPn) as InDraftPn FROM magodmis.orderscheduledetails o LEFT JOIN 
        ...
        GROUP BY d1.OrderSchDetailsID) as a on a.OrderSchDetailsID=o.SchDetailsID WHERE o.ScheduleId ='${schdetsid}'`, async (data) => {
